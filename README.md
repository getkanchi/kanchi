...
### Path prefixes and reverse proxies

Kanchi can be served behind a public URL prefix, similar to Flower's `url_prefix`
setup, but the prefix must also exist in the reverse proxy configuration.

Set `NUXT_PUBLIC_URL_PREFIX=/kanchi` when users access Kanchi through a prefixed
public URL such as:

```txt
https://example.com/kanchi/ui
```

This changes the browser-side API and WebSocket URLs to:

```txt
https://example.com/kanchi/api/...
wss://example.com/kanchi/ws
```

The FastAPI app still serves its internal routes without that prefix:
...
Configure your reverse proxy or ingress so the public prefix is stripped before
requests are forwarded to Kanchi:

```txt
public /kanchi/ui       -> upstream /ui
public /kanchi/api/...  -> upstream /api/...
public /kanchi/ws       -> upstream /ws
```

`NUXT_PUBLIC_URL_PREFIX` only controls the URLs generated by the frontend. It
does not remount the backend routes by itself.
